...
export class MqttService {
  ...
  constructor(
    @Inject(MQTT_CONFIG) config: MqttConfig,
  ...
  connect(config: IClientOptions): void {
    ...
    this._client = this._mqttMock
      ? this._mqttMock.connect(
          null,
          config
        )
      : mqtt.connect(
  ...
  ): Observable<SubscriptionGrant | T> {
        ...
        return fromPromise(
          new Promise((resolve, reject) => {
            this._client.subscribe(
              topic,
              options,
              (error: Error, granted: Array<ISubscriptionGrant>) => {
                if (error) {
                  reject(error);
                } else {
                  ...
                  const grantedWorkaround =
                    granted.length > 0
                      ? new SubscriptionGrant(granted[0])
                      : new SubscriptionGrant({
                          topic,
                          qos
                        });
          ...
          switchMap((granted: SubscriptionGrant) =>
            concat(of(granted), this.addTopic<T>(topic, granted))
          )
        );
      })
    );
